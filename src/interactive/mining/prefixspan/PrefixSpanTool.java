    ...
    public SingleSeqWrapup generateBitMapTree() {
        ...
        for(FreqSequence seq: totalFrequentSeqs){
            if (seq.getItemPairList().size() > 0) {
                FreqPatternWrapup currentFPBM = generateFreqPatternWithRelation(seq);
                newFreqSeqTree.addFreqSeq(currentFPBM);
            }
        }
        return newFreqSeqTree;
    }

    public FreqPatternWrapup generateFreqPatternWithRelation(FreqSequence currentFreqSeq) {

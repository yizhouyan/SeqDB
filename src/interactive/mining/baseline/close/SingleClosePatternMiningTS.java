...
import interactive.mining.baseline.maximum.PrefixSpanToolWithFirstReplicateTS;
...
public class SingleClosePatternMiningTS {
	...
	public SingleClosePatternMiningTS(InputSequenceWithTS inputSequence, LocalParameterSpaceWithTS localParameterSpace) {
	...
	public void updateSequenceLongToShort() {
		...
		while (currentLength > 1 && currentIndexInFS < this.totalFrequentSeqs.size()) {
			...
			while (currentIndexInFS < this.totalFrequentSeqs.size()
				...
				int curSupport = curFSObj.getItemPairList().get(0).index.size();
				ArrayList<String> curFreqSeq = new ArrayList<String>(Arrays.asList(curFSObj.getFreqSeqInString().split(",")));

				if(!this.isSubString(this.FreqSeqsInMap, curFreqSeq, curSupport)){
	...
	public boolean isSubString(HashMap<ArrayList<String>, FreqSequence> existingFreqSeqs, ArrayList<String> curFreqSeq,
							   int curSupport) {
		for (Map.Entry<ArrayList<String>, FreqSequence> existingFS : existingFreqSeqs.entrySet()) {
			if (this.strArrayContains(existingFS.getKey(), curFreqSeq) &&
					existingFS.getValue().getItemPairList().get(0).index.size() == curSupport) {
				return true;
			}
		}
		return false;

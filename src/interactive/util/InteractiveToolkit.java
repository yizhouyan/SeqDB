...
public class InteractiveToolkit {
    ...
    public static Integer [] computeNewBitSet(BitSet oldBitSet, Integer [] removePositions){
        int [] oldBitArray = InteractiveToolkit.bitSetToIntArray(oldBitSet);
        List<Integer> oldBitList = Arrays.stream(oldBitArray).boxed().collect(Collectors.toList());
        List<Integer> removeObjs = new ArrayList<Integer>();
        for(Integer i: removePositions){
            removeObjs.add(oldBitList.get(i));
        }
        oldBitList.removeAll(removeObjs);
        return oldBitList.toArray(new Integer[oldBitList.size()]);
    }
